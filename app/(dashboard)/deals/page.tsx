...
export default function ContactsPage() {
  ...
  const [viewingLead, setViewingLead] = useState<Deal | null>(null)
  // Whether the view dialog is in "edit mode"
  const [isEditing, setIsEditing] = useState(false)
  // Editable draft inside the view dialog
  const [editDraft, setEditDraft] = useState<Deal | null>(null)
...
//   }), [leads])
  ...
  leads.filter((l) => {
    const q = `${l.id} ${l.deal_name} ${l.client_id} ${l.primary_contact_id} ${l.stage}`.toLowerCase()
...
const handleDeleteLead = async (id: string) => {
  ...
  async function loadLeads(showLoader = false) {
    ...
    try {
      ...
      console.log("Fetched leads:", data.deals)
      setLeads(data.deals ?? [])
    } catch {
          ...
          <table className="lp-table">
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                <tr key={lead.id} className="lp-row" style={{ cursor: "pointer" }}  onClick={() => router.push(`/deals/${lead.id}`)}>
                   {/* <Link href={`/deals/${lead.id}`}> */}
                  {/* Contact ID */}
                  <td className="lp-id">{lead.id}</td>
                  {/* Contact Name */}
                  <td className="lp-id">{lead.deal_name}</td>
                  {/* Contact email */}
                  <td className="lp-muted">{lead.client.company_name}</td>
                   {/* Contact designation */}
                  <td className="lp-muted">{lead.primary_contact.name}</td>
                  {/* Contact phone */}
                  <td className="lp-muted">{lead.stage}</td>
                  {/* Contact phone */}
                  <td className="lp-muted">{lead.expected_value}</td>
                  {/* <td>
                    <span
                      className="lp-badge"
                      style={{
